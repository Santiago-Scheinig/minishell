...
/*   Updated: 2025/10/10 07:29:43 by sscheini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	hdoc_dup(int heredoc_fd[2], t_token *limit, t_body *msh)
{
	...
	while (1 && ++msh->line)
	{
		if (msh->interactive && write(STDERR_FILENO, "> ", 2) == -1)
			return (shell_hdocerr(limit->str, "write", heredoc_fd, msh));
		line = ft_get_next_line(STDIN_FILENO);
		if (!line)
			return (shell_hdocerr(limit->str, "read", heredoc_fd, msh));
...
static int	token_hdoc(t_token *aux_token, t_token *next_token, t_body *msh)
{
	...
	if (!msh->exit_no && aux_token->type == HEREDOC)
	{
		if (pipe(aux_fd) == -1)
		{
			...
			shell_redirerr(NULL, msh->exit_no);
			return (MSHELL_SUCCESS);
		}
		if (shell_sigign_all())
...
int	parser_token(char **split, t_body *msh)
{
	...
	while (split[++i])
	{
		new_token = shell_newtkn(split[i]);
		if (!new_token)
			break ;

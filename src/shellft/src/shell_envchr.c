...
/*   Updated: 2025/11/03 17:45:21 by sscheini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*shell_envchr(int *exported, const char *name, t_list *lst_envp)
{
	...
	shell_envsort(&lst_envp);
	while (lst_envp)
	{
		if (!lst_envp->content)
			return (NULL);
		envar_len = ft_strlen(((t_var *) lst_envp->content)->name);
		name_len = ft_strlen(name);
		envar = (t_var *) lst_envp->content;
		if (envar_len == name_len
			&& !ft_strncmp(envar->name, name, ft_strlen(name)))
		{
			if (exported)
				(*exported) = envar->exported;
			if (envar->value && !envar->value[0])
				return (NULL);
			return (envar->value);
		}

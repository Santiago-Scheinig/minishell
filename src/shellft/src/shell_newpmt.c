...
/*   Updated: 2025/10/21 20:13:22 by sscheini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*pwd_name(char *ps1, char *path, t_list *lst_envp)
{
	...
	user = shell_envchr(NULL, "USER", lst_envp);
	if (!user)
		user = NULL;
...
static char	*short_path_name(char *path, t_list *lst_envp)
{
	...
	home = shell_envchr(NULL, "HOME", lst_envp);
	if (!home)
		return (path);
	i = ft_strlen(home);
...
static char	*path_cwd(t_list *lst_envp)
{
	...
	path = getcwd(NULL, 0);
	if (!path)
		path = ft_strdup(shell_envchr(NULL, "PWD", lst_envp));
	if (!path)
		return (NULL);
...
char	*shell_newpmt(t_list *lst_envp)
{
	...
	ps1 = shell_envchr(NULL, "PS1", lst_envp);

...
/*   Updated: 2025/11/03 17:45:39 by sscheini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	cmdlog_init(char ***envp, t_list *lst_envp)
{
	int		exp;
	char	*cmdlog;
...
static int	shlvl_init(char ***envp, t_list *lst_envp)
{
	char	*shlvl;
	int		exp;
	...
	exp = false;
	shlvl = "SHLVL=1";
	if (shell_envchr(&exp, "SHLVL", lst_envp))
		return (MSHELL_SUCCESS);
	else
	{
...
static int	pwd_init(char ***envp, t_list *lst_envp)
{
	char	*pwd;
	int		exp;
	...
	if (!pwd)
		return (MSHELL_FAILURE);
	if (shell_envchr(&exp, "PWD", lst_envp))
	{
...
static int	ps1_init(char ***envp, t_list *lst_envp)
{
	...
	exp = false;
	ps1 = "PS1=\\u:\\w\\$ ";
	if (shell_envchr(&exp, "PS1", lst_envp))
	{
...
{
	(*env_des) = shell_envdup((const char **) env_src);
	if (!(*env_des))
		return (MSHELL_FAILURE);
	(*env_head) = shell_envdup_lst(env_src);
	if (!(*env_head))
		return (MSHELL_FAILURE);
	shell_envsort(env_head);
